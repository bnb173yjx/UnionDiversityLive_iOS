...
@property STFilterVC        *stFilterVC;
...
- (void)viewDidLoad {
    ...
    [self setupUI];
    [self setupLogo];
    [self.stFilterVC setFilter:_kit.previewDimension];
    [self onCapture];
}
...
- (void)setupUI{
    ...
    // bottom view
     _stFilterVC = [[STFilterVC alloc] init];
    [_stFilterVC addview:_ctrlView];
    _stFilterVC.viewUpdateCallback = ^{
       selfWeak.kit.staticStickerPic = [[UnionGPUViewCapture alloc] initWithView:selfWeak.stFilterVC.commonObjectContainerView];
       [selfWeak.kit.staticStickerPic update];
    };
